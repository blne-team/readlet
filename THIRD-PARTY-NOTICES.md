...
Readlet is MIT — see [LICENSE](LICENSE). That covers the code in this
...
`apps/readlet/scripts/pdfjs-assets.ts` copies part of `pdfjs-dist` into
`apps/readlet/public/pdfjs/` when you build, because pdf.js fetches those
...
`pdftoppm` does not make Readlet GPL. Both are optional: without them the sync
