...
COPY apps/readlet/package.json apps/readlet/
...
COPY docker/readlet.config.json ./readlet.config.json
RUN pnpm build
# Drop development dependencies after the build, keeping the workspace links
# and compiled packages in the tree copied to the runtime stage.
RUN pnpm install --prod --offline --frozen-lockfile --store-dir /pnpm-store
...
CMD ["node", "apps/readlet/node_modules/next/dist/bin/next", "start", "apps/readlet", "-H", "0.0.0.0"]
