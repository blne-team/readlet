 ...
 * lives in @readlet/core rather than being declared here and there. Re-exported
 * so the rest of the app still has one place to import a book from.
 */
export type { Book, BookFormat, Catalog } from "@readlet/core";
export { bookKey } from "@readlet/core";
 ...
 */
const CACHE_KEY = "https://readlet.internal/catalog/shelf";
...
export class CatalogService {
  ...
  /** Makes a catalog mutation visible in this isolate and every shared cache. */
  async invalidate(): Promise<void> {
    resetCatalogMemo();
    await optional(() => this.cache.remove(CACHE_KEY));
  }
}
