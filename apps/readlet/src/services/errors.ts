 ...
 * unpublished catalog is an empty shelf and a book nobody has opened has no
 * saved position. Both are normal and neither is an error. Identity state is
 * deliberately stricter: a missing directory is bootstrap state, never an
 * implicit user.
 *
 ...
 * the right answer genuinely differs: a page may render a limited outage view,
 * while user-directory failure must stop authorization.
 */
 ...
 * nonsense are different problems, and malformed state surfaces as an error.
 */
