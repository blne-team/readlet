...
export function memoryStorage(
  ...
  const revisions = new Map<string, number>(
    [...held.keys()].map((key) => [key, 1]),
  );
  const replace = (key: string, bytes: Uint8Array): void => {
    held.set(key, bytes);
  ...
  const describe = (key: string): StoredObject => ({
    ...
    etag: `"${revisions.get(key) ?? 0}-${bytesAt(key).byteLength.toString(16)}"`,
  ...
  if (writable) {
    storage.putStream = async (key, body, _size, contentType) => {
      writes.push({ key, contentType });
      refuse("putStream");
      replace(key, new Uint8Array(await new Response(body).arrayBuffer()));
    };
    ...
    storage.writeIf = async (key, bytes, expectedEtag, contentType) => {
      writes.push({ key, contentType });
      refuse("writeIf");
      const actual = held.has(key) ? describe(key).etag : null;
...
export function recordingCache(): {
  calls: { op: "match" | "put" | "remove"; key: string; hit?: boolean }[];
  size: () => number;
  cache: ResponseCache;
} {
