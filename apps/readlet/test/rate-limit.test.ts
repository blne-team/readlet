...
import { USERS_FILE } from "@readlet/core";
 ...
 * Worker is reachable by authenticated users, and the bucket's allowance is finite.
 *
...
let access: Awaited<ReturnType<typeof startAccessFixture>>;
...
function ask<C>(
  ...
  return route(
    new Request(`https://shelf.test/x/${key}${query}`, { ...init, headers }),
    {
...
test("a visitor over the allowance is refused before the bucket is read", async () => {
  ...
  assert.deepEqual(
    memory.reads.filter((read) => read.key !== USERS_FILE),
    [],
  );
...
test("a request that fails at the bucket has still spent its allowance", async () => {
  ...
  const { limits, asked } = recordingLimits();
  setServices(createServices(memory.storage, nullCache(), limits));
  const read = memory.storage.read.bind(memory.storage);
  memory.storage.read = async (key, options) => {
    if (key === BOOK) throw new Error("book storage is unreachable");
...
test("a key that names no book never reaches the limiter", async () => {
  ...
  const response = await ask(download, ".readlet/users.json", from(VISITOR));
...
test("a chapter read out of the archive is counted", async () => {
  const memory = memoryStorage({ ...userDirectory(), [BOOK]: TEXT });
  const { cache } = recordingCache();
  const { limits, asked } = recordingLimits();
  setServices(createServices(memory.storage, cache, limits));
...
test("a broken limiter fails explicitly", async () => {
