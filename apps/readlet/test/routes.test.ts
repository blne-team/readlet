...
const POSITIONS = ".readlet/progress/someone.json";
...
test("private bytes require an Access token before reading storage", async () => {
  const memory = library();
  for (const response of [
    await ask(download, BOOK, { token: "" }),
...
test("a verified but uninvited identity cannot read private bytes", async () => {
  const memory = library();
  const token = await access.tokenFor("cf-uninvited", "stranger@example.com");
  for (const response of [
    await ask(download, BOOK, { token }),
    await ask(cover, COVER, { token }),
  ]) {
  ...
  assert.deepEqual(
    memory.reads.map((read) => read.key),
    [USERS, USERS],
  );
});
...
test("the app's own state is not downloadable", async () => {
  ...
  for (const key of [USERS, POSITIONS, "catalog.json"]) {
  ...
  // no read at all.
  assert.deepEqual(
    memory.reads.filter((read) => read.key !== USERS),
    [],
  );
});
...
test("the app's own state is not a cover either", async () => {
  library();
  assert.equal((await ask(cover, USERS)).status, 404);
});
