...
/** The package directory resolved from this app's declared dependency. */
 ...
 */
const files = ["legacy/build/pdf.worker.min.mjs"];
const directories = [
  { source: "cmaps", target: "cmaps" },
  { source: "standard_fonts", target: "standard_fonts" },
  { source: "wasm", target: "wasm" },
  { source: "iccs", target: "iccs" },
  { source: "web/images", target: "images" },
];
...
/** Changes whenever either PDF.js or the set of published runtime files does. */
function fingerprint(): string {
  return [
    version(),
    ...files,
    ...directories.map(({ source, target }) => `${source}:${target}`),
  ].join("\n");
}
...
async function main(): Promise<void> {
  ...
  if ((await current()) === wanted) {
    console.log(`pdf.js ${version()} assets are already in place`);
    return;
  }
  ...
  for (const directory of directories) {
    await cp(
      ...
      path.join(target, directory.target),
      {
        recursive: true,
  ...
  console.log(`Copied pdf.js ${version()} assets into public/pdfjs`);
}
