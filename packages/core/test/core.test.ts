...
test("state lives under one reserved prefix", () => {
  ...
  // nobody publishes a book called "readlet".
  assert.ok(USERS_FILE.startsWith(STATE_PREFIX));
  assert.ok(progressFile("reader").startsWith(STATE_PREFIX));
  assert.ok(LIBRARY_OPERATIONS_FILE.startsWith(STATE_PREFIX));

  assert.equal(isStateKey(USERS_FILE), true);
  ...
  assert.equal(isStateKey("readlet/a.epub"), false);
});
...
test("a key from a request names a file in a book's folder, or nothing", () => {
  ...
  assert.equal(parseBookKey(USERS_FILE), null);
  ...
  assert.equal(parseBookKey("../catalog.json"), null);
  assert.equal(parseBookKey("dracula/../.readlet/users.json"), null);
  assert.equal(parseBookKey("dracula/.."), null);
  assert.equal(parseBookKey(".readlet/users.json"), null);
...
test("a user id is constrained because it becomes a file name", () => {
  assert.equal(isUserId("u-reader"), true);
  assert.equal(isUserId("murat-erkin"), true);
  assert.equal(isUserId("a1"), true);
  assert.equal(isUserId("9lives"), true);
  ...
  assert.equal(isUserId("-leading"), false);
  assert.equal(isUserId(""), false);
  assert.equal(isUserId("Upper"), false);
  assert.equal(isUserId("has space"), false);
  assert.equal(isUserId("has_underscore"), false);
  assert.equal(isUserId("../escape"), false);
  assert.equal(isUserId("a/b"), false);
  assert.equal(isUserId("a".repeat(65)), false);
  assert.equal(isUserId("a".repeat(64)), true);
});
...
test("a catalogued book is the authority for its storage keys", () => {
  assert.deepEqual(
    bookObjectKeys({
      id: "a-book",
      title: "A Book",
      ...
      cover: "cover.webp",
      formats: [
        { format: "epub", file: "a-book.epub", size: 12 },
        { format: "pdf", file: "a-book.pdf", size: 34 },
      ],
    }),
    [
  ...
  assert.throws(() =>
    bookObjectKeys({
      ...
      title: "A Book",
      authors: [],
      formats: [{ format: "epub", file: "../private.json", size: 1 }],
    }),
  );
});
...
test("writable storage is decided by asking, not by trying and catching", () => {
  ...
  // A partial write path is not writable app state.
...
test("read-only storage carries no write path at all", () => {
  ...
  assert.equal(readOnly.writeIf, undefined);
