...
/**
 * Every object that belongs to one catalogued book.
 *
 * The catalog is the authority for a book's files. Keeping this in core makes
 * publishing and in-app library management agree about that boundary, and the
 * validation prevents a malformed catalog from turning cleanup into deletion
 * of an arbitrary storage key.
 */
export function bookObjectKeys(book: Book): string[] {
  if (
    !book ||
    typeof book.id !== "string" ||
    !Array.isArray(book.formats) ||
    (book.cover !== undefined && typeof book.cover !== "string") ||
    book.formats.some((format) => !format || typeof format.file !== "string")
  ) {
    throw new Error("The catalog contains an invalid book.");
  }

  const files = [
    METADATA_FILE,
    ...(book.cover ? [book.cover] : []),
    ...book.formats.map((format) => format.file),
  ];

  return [...new Set(files)].map((file) => {
    const key = bookKey(book.id, file);
    const parsed = parseBookKey(key);
    if (!parsed || parsed.id !== book.id || parsed.file !== file) {
      throw new Error(`The catalog contains an invalid file for ${book.id}.`);
    }
    return key;
  });
}
...
export function parseBookKey(key: string): { id: string; file: string } | null {
  ...
  // what keeps `progress/<user>.json` from being reachable as a file — and
