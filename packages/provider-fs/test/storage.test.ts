...
import { createAdmin, createStorage } from "@readlet/provider-fs/node";

/** A library holding one object, and the storage that reads it. */
async function library(contents = "0123456789") {
  const directory = await mkdtemp(path.join(tmpdir(), "readlet-fs-"));
...
test("conditional state writes across filesystem instances keep one winner", async () => {
  const { storage: first, directory } = await library();
  const second = createStorage({ directory, projectRoot: directory });
  const key = ".readlet/reader-marks/reader.json";
  const one = new TextEncoder().encode('{"mark":"one"}');
  const two = new TextEncoder().encode('{"mark":"two"}');
  const results = await Promise.all([
    first.writeIf(key, one, null),
    second.writeIf(key, two, null),
  ]);
  assert.deepEqual(results.sort(), [false, true]);

  const current = await first.head(key);
  assert.ok(current);
  assert.equal(await second.writeIf(key, one, '"stale"'), false);
  assert.equal(await second.writeIf(key, one, current.etag), true);
  assert.equal(
    new TextDecoder().decode(await first.readBytes(key)),
    '{"mark":"one"}',
  );
});
...
async function published() {
  ...
  await write(".readlet/users.json", '{"version":1,"users":[]}');
  await write(".readlet/progress/reader.json", '{"books":{}}');
...
test("a file that cannot be read is not reported as absent", async () => {
  ...
  try {
    // What the services read a catalog, a user directory and a position with.
