...
import { writeZip } from "@readlet/fixtures";
import { readEpub } from "@readlet/sync/epub";

/**
 * An EPUB built around one package document.
 *
 * Deliberately lower-level than @readlet/fixtures's `epub()`, which builds plausible
...
<container xmlns="urn:oasis:names:tc:opendocument:xmlns:container" version="1.0">
  ...
  const directory = await mkdtemp(path.join(tmpdir(), "readlet-epub-"));
