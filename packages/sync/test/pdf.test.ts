...
import { bytesSource, readPdfMetadata } from "@readlet/core";
import {
  classicPdf,
  literal,
  objectStreamPdf,
  xmpPacket,
} from "@readlet/fixtures";
import {
  findIsbn,
  isJunkTitle,
  splitAuthors,
  splitSubjects,
  xmpValues,
} from "@readlet/sync/metadata";
import { readPdf } from "@readlet/sync/pdf";
...
async function fixture(name, bytes) {
  const directory = await mkdtemp(path.join(tmpdir(), "readlet-test-"));
