 ...
 * Its names and shape come from @readlet/core, because the app's build reads
...
async function readJsonc(file: string): Promise<ReadletConfig> {
  return parseJsonc<ReadletConfig>(await readFile(file, "utf8"));
}
...
async function readIfPresent(file: string): Promise<ReadletConfig | null> {
  try {
...
async function findConfig(
  from: string,
): Promise<{ file: string; config: ReadletConfig; root: string } | null> {
  let directory = path.resolve(from);
