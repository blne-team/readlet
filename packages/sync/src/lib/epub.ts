...
export async function readEpub(file: string): Promise<ReadBook> {
  ...
  return readEpubSource(bytesSource(await readFile(file)));
}

export async function readEpubSource(source: ByteSource): Promise<ReadBook> {
      ...
      if (body) {
        cover = {
          ...
          extension:
            ...
            (coverPath.match(/\.[^./]+$/)?.[0]?.toLowerCase() || ".jpg"),
        };
      }
    }
  }
